...
class DataGenerator(object):
    ...
    def __init__(self, base_path, input_folder, output_folder, node_num, test_ratio=0.3, val_ratio=0.2):
    ...
    def get_neg_edge_samples(self, pos_edges, edge_num, all_edge_dict):
        """
        :param pos_edges: 测试或验证等边的数组集合
        :param edge_num: 输入边集的数量
        :param all_edge_dict: 字典，边为键，值为1
        :return: 输入的正常边集合（pos_edges）和负采样得到的边的垂直拼接。
        """
        neg_edge_dict = dict()
        neg_edge_list = []
        cnt = 0
...
class LinkPredictor(object):
    ...
    def get_edge_feature(self, edge_arr, embedding_arr):
        """
        :param edge_arr: 输入训练集或其他集的边表，含正负采样的边。二维数组。当前时间步的边表，相对于这里的embedding为下一个时间步
        :param embedding_arr: 输入上一个时间步的embedding
        :return: feature_dict
        """
        avg_edges, had_edges, l1_edges, l2_edges = [], [], [], []
        for i, edge in enumerate(edge_arr):    ##这里的edge为单条边和label，如：[24526  2416     1]
    ...
    def train(self, train_edges, val_edges, embeddings):
        """
        :param train_edges: 之前保存的训练集（含正负采样边）。当前时间步的边表，相对于这里的embedding为下一个时间步
        :param val_edges:  之前保存的验证集（含正负采样边）
        :param embeddings: 上一个时间步的embedding
        :return:返回每种测试如L1和L2中最好的结果，字典
        """
    ...
    def test(self, test_edges, embeddings, model_dict, date):
        """
        :param test_edges:之前保存的测试集（含正负采样边）。当前时间步的边表，相对于这里的embedding为下一个时间步
        :param embeddings: 上一个时间步的embedding
        :param model_dict: 训练集返回的字典，键为['Avg', 'Had', 'L1', 'L2']，
        :param date: 当前时间步边表的名字。（这里除去了后缀）
        :return:
        """
        ...
        print('the auc_list is: {}'.format(auc_list))
        return auc_list

    def link_prediction_all_time(self, method):
        """
        :param method: 输入该方法的方法列表
        :return:
        """
        print('method = ', method)
